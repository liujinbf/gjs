...
🔴【贵金属时机判断纪律】
1. 先判断能不能做（盈亏比 + 多周期共振），再判断往哪边做。
2. 若多周期共振失效（小周期 vs 大周期方向相反），胜率低于 50%，直接观望。
3. 若白银波动更剧烈、点差更宽、节奏更乱，明确写出"白银不适合普通用户追单"。
4. 任何建议都要带"下一次复核节点"，比如等数据落地后再评估。
""".strip()
