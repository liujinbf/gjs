...
_STATE_FILE_OVERRIDES: dict[str, dict] = {}
    ...
    if state_file is not None:
        ...
        if target.exists():
            try:
                ...
                pass
        cached = _STATE_FILE_OVERRIDES.get(str(target))
...
def _write_state(
    ...
    if state_file is not None:
        ...
        target = Path(state_file)
        _STATE_FILE_OVERRIDES[str(target)] = dict(state)
        try:
            target.parent.mkdir(parents=True, exist_ok=True)
            target.write_text(json.dumps(state, ensure_ascii=False, indent=2), encoding="utf-8")
        except OSError:
            # 沙箱/临时目录权限受限时，测试路径退回到进程内覆盖态，不影响生产 SQLite 路径。
            pass
        return
